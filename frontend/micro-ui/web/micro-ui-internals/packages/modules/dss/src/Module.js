...
const DssBreadCrumb = ({ location }) => {
  ...
  const crumbs = [
    ...
    {
      path:
        checkCurrentScreen() || window.location.href.includes("NURT_DASHBOARD")
    ...
    {
      path: fromModule
        ? `/digit-ui/employee/dss/dashboard/${fromModule}`
        : `/digit-ui/employee/dss/dashboard/${Digit.Utils.dss.getCurrentModuleName()}`,
